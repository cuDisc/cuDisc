...
CSR_SpMatrix _create_FLD_matrix_1p1D(const Grid& g, int num_bands) {
    int nx = g.NR ;
    int ny = g.Nphi ;
...
__global__ void compute_diffusion_coeff_1p1D(GridRef g, Field3DConstRef<double> J,
    ...
    if (i < g.NR + 2*g.Nghost && j < g.Nphi + 2*g.Nghost) {
        for (int k=0; k < J.Nd; k++) {
            ...
            D(i,j,k) = diffusion_coeff_1p1D(g, J, krho, i, j, k) ;
        }
    }
}

__global__ void compute_diffusion_coeff_1p1D(GridRef g, FieldConstRef<double> J,
                                             FieldConstRef<double> rhokabs,  
                                             FieldConstRef<double> rhoksca,
                                             FieldRef<double> D) {
    ...
    if (i < g.NR + 2*g.Nghost && j < g.Nphi + 2*g.Nghost) {
        double krho = rhokabs(i,j) + rhoksca(i,j) ;          
        D(i,j) = diffusion_coeff_1p1D(g, J, krho, i, j) ;
    }
}


__global__ void create_FLD_system_1p1D(GridRef g, double dt, double Cv, 
    ...
    if (i < nx+g.Nghost && j < ny+g.Nghost) {
        ...
        double Dij[3];
        compute_diffusion_matrix_1p1D(g, D, Dij, boundary, i, j, b) ;
        ...
        if (j == g.Nghost) {
            if (boundary & BoundaryFlags::open_Z_inner)
                rhs.data[idx] -= J_ext * Dij[0] ;
        }
        if (j == ny + g.Nghost - 1) {
            if (boundary & BoundaryFlags::open_Z_outer)
                rhs.data[idx] -= J_ext * Dij[2] ;
        }
        ...
        mat.data[start] += Dij[1] ;
        start++;

        if (j < ny + g.Nghost - 1) {
            mat.col_index[start] = idx + (1+num_bands) ;
            mat.data[start] = Dij[2] ;
            start++ ;
        }
...
void FLD_Solver::solve_multi_band_1p1D(const Grid& g, double dt, double Cv, 
    ...
    CodeTiming::BlockTimer timing_block = 
        timer->StartNewTimer("FLD_Solver::solve_multi_band_1p1D") ;
    CodeTiming::BlockTimer timing_subblock = 
        timer->StartNewTimer("FLD_Solver::solve_multi_band_1p1D::create_system") ;   

    dim3 threads(32,32,1) ;
    dim3 blocks((g.Nphi + 2*g.Nghost+31)/32,(g.NR + 2*g.Nghost+31)/32,1) ;
          
    CSR_SpMatrix FLD_mat = _create_FLD_matrix_1p1D(g, J.Nd) ;
    ...
    compute_diffusion_coeff_1p1D<<<blocks, threads>>>(g, J, rhokappa_abs, rhokappa_sca, D) ;
    ...
    create_FLD_system_1p1D<<<blocks2, threads2>>>(g, dt, Cv, rho, rhokappa_abs, T, J, D, 
                                                        heat, scattering,
                                                        _T_ext, planck, wle.get(),  
                                                        FLD_mat, rhs, _boundary) ;
    check_CUDA_errors("create_FLD_system_1p1D") ;    

    timing_subblock.StartNewBlock("FLD_Solver::solve_multi_band_1p1D::solve") ;      
